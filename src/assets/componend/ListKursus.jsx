      ...
      <div className="bg-[#F1FEFF]">
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-2 text-lg leading-8 text-gray-600">Kesempatanmu ada di depan mata masuk PTN Terbaik.</p>
          </div>
              ...
              programs.map((product, index) => (
                <div key={index} className="w-full">
                  <div className="relative">
                    <div className="relative h-72 w-full overflow-hidden rounded-lg">
                      <img src={product.imageURL} alt={product.imageURL} className="h-full w-full object-cover object-center" />
                    </div>
                ...
                >
                  Daftar<span className="sr-only">, {product.name}</span>
                    ...
                    <button
                      onClick={() => handleBuyKursus(product.id)}
                      className={`w-full relative flex items-center justify-center rounded-md border border-transparent px-8 py-2 text-sm font-medium text-white ${
                        isEnrolled(product.id) ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-500 hover:bg-indigo-600'
                      }`}
                      disabled={isEnrolled(product.id)}
                    >
                      <span className="sr-only"> </span>
                      {isEnrolled(product.id) ? 'Sudah Terdaftar' : 'DAFTAR'}
                    </button>
                  </div>
                </div>
              ))}
